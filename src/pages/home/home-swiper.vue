<template>
  <div class="homeSwiperDivType" id="certify">
    <!-- <div class="titleType">
      ...
      <div class="swiper-wrapper">
        <div class="swiper-slide" v-for="(item, index) in data" :key="item.id">
          <div :class="[index%2 === 0? animationData.leftIn : animationData.rightIn]"
            @click.stop.prevent="handlerSwiper($event, item, index)">
    ...
    <div class="bottomType" v-for="(item,index) in data" :key="item.id">
      ...
      <div v-if="index === btnShow">
        <!-- <div v-if="true"> -->
        <Button class="btnType" v-for="o in item.childList" :key="o.id"
          @click="handlerMenu('/product/'+item.code+'/'+o.code)">{{o.name}}</Button>
      </div>
    </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      handlerMenu(val, isGo) {
        ...
        if (!isGo) {
          ...
          if (pathVal2.length === 1) {
            ...
            if (pathVal2[1] === "/deals" || pathVal2[1] === "/mobileDevelop") {
              ...
              console.log("mobileDevelop", val, pathId);
              // 网站建设，去子项，二级菜单
              if (pathId) {
                const gotoPath = pathVal2[0] + pathVal2[1];
                this.$go({
      ...
      goSwiper(val) {
        this.setCertifySwiper().slideTo(index, 1000, false)
      },
      handlerSwiper(e, val, index) {
        console.log("e", e)
        console.log("valval", val)
      ...
      setCertifySwiper() {
        const that = this;
        return new Swiper('#certify .swiper-container', {
          ...
          onProgress: function (swiper, progress) {
            ...
            // 选择的当前的数
            let chooseNum = Math.round(progress * (swiper.slides.length - 1));
